...
function handleInputChange(event) {  //FUNCION PARA OBTENER EL ID O NOMBRE DEL POKEMON QUE SE HA INTRODUCIDO EN EL INPUT
    const inputValor = event.target.value;
    buscar(inputValor);
}

function buscar(nombre) { //FUNCION QUE SE EJECUTARA CADA VEZ QUE HAYA UN CAMBIO EN EL INPUT
    ...
    fetch(`https://pokeapi.co/api/v2/pokemon/${nombreFormateado}`) //HACEMOS EL LLAMADO A LA API
        .then((response) => {
            if (response.status == 404) { //EN CASO DE QUE EL ESTADO DE LA RESPUESTA SEA 404, LO CUAL SIGNIFICA QUE EL ID O NOMBRE DEL POKEMON NO ES VALIDO, ARROJAREMOS UN ERROR
        ...
        .then((pokemon) => {
            if (pokemon !== undefined && nombre) { //VERIFICAMOS QUE LA RESPUESTA NO SEA UNDEFINED Y QUE EXISTE UN NOMBRE
                ...
                pokemonImage.setAttribute(      // ACTUALIZAMOS LA FUENTE DE LA IMAGEN QUE USAREMOS (SRC) CON EL NUEVO LINK
                    "src",
                ...
                pokemon.types.forEach((element) => {                    //RECORREMOS EL ARRAY QUE NOS DA LA API, POR CADA TIPO CREAREMOS SU RESPECTIVA TARJETA
